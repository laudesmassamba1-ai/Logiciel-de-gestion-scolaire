...
### Correctifs post-livraison 1.6.0 (PDF, paiement MTN/Airtel, surface noire, emblèmes, Charo) + rebuild final — branche `ci/build-1.6.0-weasy`
- **Contexte** : suite aux remarques utilisateur après la livraison 1.6.0 initiale : (1) les PDF générés ne s'ouvrent pas (lecteur par défaut `evince` dans un environnement dégradé), (2) le mode de paiement affiche encore « Moov » (opérateur renommé Airtel), (3) fenêtre d'inscription élève : zone de défilement noire, (4) vouloir régler l'emplacement/size des emblèmes sur les PDF, (5) la navigatrice IA Charo ne connaît pas les nouvelles pages.
- **Correctifs code** (commit `aca7623`, 11 fichiers, +245/−29) :
  - `services/pdf_export.py` : `_generate_pdf` attrape toute exception weasyprint → `RuntimeError` lisible ; **`_ouvrir_pdf` robuste multi-OS** (`os.startfile` Windows / `open` détaché macOS / `QDesktopServices` puis `xdg-open` détaché timeout 2 s Linux / dernier recours : message chemin + ouverture du dossier parent) ; nouvelles `_reglage_embleme()` + `_entete_doc()` paramétrables (alignement gauche/centre/droite + hauteur max, bornes 30–400 px).
  - `core/config.py` : `MODES_PAIEMENT = ("Especes", "Mobile Money (MTN / Airtel)", "Cheque / Virement")` et `MODES_SORTIE` ; utilisés par `caisse_page.py` et `paiements_page.py` (plus aucun « Moov ») et champs UI `inscription.ui` (libellé Airtel).
  - `ui/ui_files/eleves/inscription.ui` + `ui/pages/eleves.py::open_inscription_dialog` : fond transparent de la zone défilante (`QScrollArea#scrollArea > QWidget > QWidget { background: transparent; }` + `viewport()/scrollContent.setAutoFillBackground(False)`) — fin de la surface noire.
  - `ui/pages/parametres_page.py` : groupe « Emplacement des emblemes (documents PDF) » → clés paramètres `{bandeau_haut|bandeau_bas|signature}_align` / `{...}_hauteur` (sauvegarde `repos.set_parametre`, rechargés par `_reglage_embleme` à chaque export).
  - `ui/pages/documents_page.py::_ouvrir` + bouton « Ouvrir » documents de `eleves.py` (l. 673) : réutilisent `pdf_export._ouvrir_pdf`.
  - `repositories/finance_repository.py` : **`_canoniser_mode()`** (unicodedata NFD : casse, accents, anciens « Moov »/« Espèces » → libellés canoniques) appliqué au filtre `paiements(mode=...)` et à `add_paiement`.
  - `services/assistant_ia.py` : `_PAGES_NAV` porté à 34 entrées (documents, pdf, rapports, bloc note(s), notes personnelles, calendrier, agenda, reseau, synchronisation, sync — « bloc note/notes personnelles » avant `note`), `_PAGES_TITRES`, manuel +2 articles (Documents & rapports PDF ; Bloc notes/calendrier/réseau), déclencheur « va dans » ajouté à `_essayer_navigation`.
  - Table `parametres` : colonnes **`cle` / `valeur`** (pas `clef`) — à respecter pour `set_parametre`.
- **Vérifications locales** : `py_compile` OK (10 fichiers) ; XML `inscription.ui` valide ; PDF réel 457 574 o généré avec alignements personnalisés et paramètres emblèmes restaurés ; `_canoniser_mode` testé (Espèces→Especes, Moov→Airtel, « Mode : Especes »→Especes…) ; navigation Charo testée (« ouvre les documents »→documents, « va dans les rapports »→rapports, « ouvre le bloc notes »→bloc_notes, « ouvre l'agenda »→calendrier) ; **313 tests desktop verts** + **63 tests IA verts** (après ajout du déclencheur).
- **Version** : utilisateur a choisi de **rester en 1.6.0** (pas de 1.6.1) ; `APP_VERSION` inchangé.
- **Rebuild CI** : 1er run `35978260715` **ÉCHEC au garde-fou** — `ImportError: libpulse-mainloop-glib.so.0` dans `tests/test_helpers.py` (imports QtMultimedia) : `libpulse0 libpulse-mainloop-glib0 libasound2` étaient installés dans les workflows dédiés mais **pas dans `build.yml`** → correctif CI (commit `3689985`) ajouté aux 2 jobs Linux de `build.yml` (test + build). Run relancé `35978465124` **SUCCESS complet** : garde-fou pytest 1m19, Linux (.deb + AppImage) 3m33, Windows (Setup + portable) 3m39.
- **Binaires finaux livrés** dans `executables/` (gitignoré) :
  - `gestion-scolaire_1.6.0_amd64.deb` (97,4 Mo) — SHA-256 `cacb13c8934dcbbbe2231b9723e795e25f9a746f01040f99d2c0b84ee6090e32`
  - `GestionScolaire.exe` onefile (15,1 Mo) — SHA-256 `4bfe617ceeaa0ae20a2b6af49fb0c3c8bd751d10991be773c7c9b37059f6b3e5` (build_windows.yml run `35979617368` SUCCESS, seul à produire le onefile ; `build.yml` ne produit que Setup + zip portable)
  - `GestionScolaire-Setup-1.6.0.exe` (60,7 Mo) — SHA-256 `d5a93eb4e7fbe399cae8b25bae9a8f8a4b6552d7e3807b6ed1ac6a3e545d098d`

## Version 1.6.1 — causes racines enfin identifiées (bandes noires + PDF) — branche `ci/build-1.6.1`
- **Remontée utilisateur (post-1.6.0)** : (1) les bandes noires persistent dans la fenêtre d'inscription d'un nouvel élève, (2) « l'app ne génère pas les PDF demandés (reçus, certificats, etc.) ».
- **Enquête « bandes noires »** (reproduction offscreen par `QWidget.grab()` + analyse pixel Python sur `/tmp/opencode/repro_bande_noire.py`) : rien à voir avec la zone défilante (déjà corrigée en 1.6.0) ; l'écran est noir sur **les marges du dialogue (x 0–29 et 1157–1197), l'en-tête (y 0–71), les espacements entre cartes (y 303–318 et 587–602) et la zone basse (y 736–832)** — **26,5 % de pixels noirs** au total.
  - **Cause racine** : le widget **racine** du dialogue porte `QWidget#FormInscriptionEleve { background: transparent; }` → rien ne peint ces zones sur les postes où le parent n'hérite d'aucun fond.
  - **Mécanisme exact** : `apply_ui()`/`loadUi(widget)` pose la feuille de style du fichier `.ui` **sur la fenêtre contenante** (qui prend l'`objectName` du fichier) ; une règle `#… { background: transparent }` posée ainsi laisse alors la fenêtre **totalement non peinte** (le QStyleSheetStyle remplace le style natif) → le fond rendu est la palette sombre (noir). C'est pourquoi le problème apparaît quel que soit le thème/le CSS global.
  - **Correctif** (`inscription.ui`) : cette règle racine est remplacée par le dégradé aura explicite `background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #EAF2FF, stop:0.45 #F2EEFF, stop:1 #FBF4E6)` (commentaire in situ pour éviter la régression). **Re-vérifié : 0 % de pixels noirs** sur le fichier réellement modifié.
  - **Audit systématique de tous les `.ui`** (script `/tmp/opencode/audit_noir_ui.py` qui charge chaque écran, grab + comptage pixels RGB<12) : le même défaut affectait **`ui/ui_files/dashboards/dashboard_gestionnaire.ui`** (racine `QWidget#PageGestionnaire { background: transparent; }`, 67 % noir hors contexte) → **même correctif appliqué** (dégradé aura). Résultat : les 6 écrans `.ui` instrumentables à **0,00 %** de pixels noirs ; `main.ui` (QMainWindow, racine déjà peinte en dégradé) vérifié à 0,00 % via son chargement réel.
- **Enquête « PDF non générés »** :
  - **La génération fonctionne** (preuve dans le bundle installé) : `recu_ELEV20260001_REC-85DFC52D.pdf` (35 Ko) et `rapport_synthese.pdf` (36 Ko) présents dans `~/.local/share/gestion-scolaire/data/documents/` (PDF valides 1.7).
  - **Cause racine de « rien ne s'ouvre »** (logs `journalctl`) : `xdg-open: no method available for opening 'file:///…/rapport_synthese.pdf'` — le poste n'a **aucune méthode xdg-opener fiable sur la session** (les .desktop des lecteurs Absents de `/usr/share/applications` malgré `mimeinfo.cache` et `which evince` OK) ; QDesktopServices répondait vrai sans rien ouvrir (QPA actif) → l'utilisateur ne voyait jamais le PDF et concluait « non généré ».
  - **Correctif** (`services/pdf_export.py::_ouvrir_pdf` réécrit) : filet complet Linux **sans dépendre des associations MIME ni de la valeur de retour de QDesktopServices** → 1/ `xdg-open`, 2/ `gio open`, 3/ lecteurs PDF directs (`evince`, `okular`, `qpdfview`, `zathura`, `mupdf`, `atril`, `epdfview`, `xreader`) ; chaque étape **vérifie qu'un nouveau processus lecteur est réellement apparu** (`pgrep -f`, diff de PIDs avant/après, sous-processus détaché `start_new_session`), et en échec total : message avec le chemin + ouverture du dossier parent. Testé sur le poste : `evince` s'ouvre bien (PIDs détectés).
- **Version** : `APP_VERSION = "1.6.1"` dans `core/config.py` (+ alignement `server/test_routes_exercice.py`) — l'utilisateur a cette fois demandé explicitement la montée de version ; autres références « 1.6.0 » uniquement dans l'historique `docs/`.
- **Vérifs locales** : `py_compile` OK ; reproduction pixel 0 % noir ; ouverture PDF réelle via le nouveau `_ouvrir_pdf` (evince lancé) ; **313 tests desktop verts** (`pytest tests/ -q`, 3m13).
- **Build 1.6.1** : commits `342cd03` (inscription + ouverture PDF + version) et `67d63db` (dashboard) poussés sur `ci/build-1.6.1` ; premiers runs `build.yml` `36000689066` + `build_windows.yml` `36000693343` SUCCESS, **re-runs après le correctif dashboard** : `36002414065` (build.yml) + `36002417910` (build_windows.yml).
- **Vérification des binaires livrés** (rigueur) : le paquet `.deb` contient les `.ui` corrigés (dégradé aurora sur inscription et dashboard, vérifié sur les fichiers embarqués) ; le PYZ embarqué contient `services.pdf_export` avec le filet d'ouverture (`xdg-open`, `gio`, `pgrep`) et `core.config` avec `APP_VERSION = "1.6.1"` (marshal + co_consts). Suite pytest complète : **313 passed**.
- **Livraison 1.6.1** : branche distante `installers` re-poussée en force avec un nouveau commit orphelin (`5d28e8b`, uniquement `executables/`) contenant :
  - `GestionScolaire-Setup-1.6.1.exe` (60,7 Mo) — SHA-256 `12c20014c11a645bcb0fdc80c11238078eca0fa650e51281c4ead46a35a48de4`
  - `GestionScolaire.exe` (onefile 15,1 Mo, build_windows.yml) — SHA-256 `8b729b4597ec645d6aae5bb28bf8234a2b96cae94cc03da4717759eeeb23cc82`
  - `gestion-scolaire_1.6.1_amd64.deb` (97,4 Mo) — SHA-256 `a9b49c14ba22f49f06384d844279b31b103d6bb1d6e8fab01d965d26e66245fc`
  - Copies locales : `executables/` (gitignoré) et `~/Téléchargements/Gestion_Scolaire_1.6.1/` (pour historique ; la version 1.6.0 y reste).
- **Smoke test binaire Linux 1.6.1** (une fois `/tmp` désengorgé après le coup de chaud disque à 100 %) : binaire extrait du `.deb` (`dpkg-deb -x`) lancé avec `QT_QPA_PLATFORM=offscreen` et `GS_DATA_DIR` isolé → processus vivant, base `ecole.db` créée au premier lancement, arrêt propre. Démarrage validé.
- **Publication GitHub** : tag `v1.6.1` poussé sur le commit orphelin `5d28e8b` ; **release publique sans aucune note** → https://github.com/laudesmassamba1-ai/Logiciel-de-gestion-scolaire/releases/tag/v1.6.1, avec **5 assets** : `gestion-scolaire_1.6.1_amd64.deb`, `GestionScolaire-1.6.1.AppImage`, `GestionScolaire-Portable-1.6.1.zip`, `GestionScolaire-Setup-1.6.1.exe` (build_windows) et `GestionScolaire.exe` (onefile). Même schéma que la v1.6.0 (4 assets + onefile).
- **Installation utilisateur** : la 1.6.1 est installée sur le poste (`/opt/gestion-scolaire` — binaire daté, `inscription.ui` dégradé aurora présent) et utilisée. L'AppImage de son côté ne monte pas sur ce poste (FUSE non dispo) → l'utilisateur reste sur le `.deb`.
- **Livraison** : branche distante `installers` re-poussée en force avec un nouveau commit orphelin (`87217d6` « Livraison binaires Gestion Scolaire 1.6.0 (correctifs…) », uniquement `executables/`) ; branche temp `ci/build-1.6.0-weasy` supprimée du remote (conservée en local pour fusion future dans `main`) ; `installers` locale re-pointée sur `origin/installers` ; `data/alarm.wav` fait partie des ressources versionnées de la branche de travail (prévoir sa gestion lors des futurs changements de branche).

## Session XXXVI — Mises à jour distantes automatiques (options A + B) + chasse aux bugs
- **Demande utilisateur** : « dès que l'ordi est connecté à Internet et qu'on rentre dans le logiciel, est-ce que ça peut afficher qu'il y a une mise à jour, demander oui/non, et si oui télécharger et installer la mise à jour directement ? » — choix « Options **A** (GitHub) **+ B** (serveur central de l'école) ».
- **Architecture (`services/updater.py`, ~460 lignes)** : le **serveur central prime** (l'école peut diffuser/imposer), sinon on consulte les **releases GitHub**. Aucune vérification ne bloque le démarrage.
  - `version_cle` / `plus_recente` : comparaison semver stricte (`v?x.y.z`), `APP_VERSION` (`1.6.1`) comme base.
  - `cible_serveur()` (option B) : `GET /mise-a-jour/etat` via `api/client._request` **uniquement si `SYNC_ACTIVE`** ; retourne None si consigne inactive, pas plus récente ou erreur réseau (silencieux).
  - `cible_github()` (option A, repo PUBLIC `laudesmassamba1-ai/Logiciel-de-gestion-scolaire`) : release `latest` + vérification SHA-256 via un asset `SHA256SUMS.txt` quand il existe ; silencieux hors ligne / limite d'API.
  - Choix du paquet selon la machine : `gestion-scolaire_<v>_amd64.deb` (Linux), `GestionScolaire-<v>.AppImage`, `GestionScolaire-Setup-<v>.exe` (Windows) avec repli onefile `GestionScolaire.exe`.
  - `telecharger()` : flux octets dans `data_dir()/mises_a_jour/`, fichier `.part` puis `os.replace`, vérification SHA-256 (paquet corrompu supprimé), callback de progression.
  - Consigne serveur (`fichier_consigne` = `data_dir()/mise_a_jour.json` → endpoint `/mise-a-jour/etat`) : `actif`, `version`, `obligatoire`, `paquet_linux`, `paquet_windows`. `deposer_paquet()` copie un paquet dans `data_dir()/mises_a_jour_paquets/` (endpoint `/mise-a-jour/paquet/{nom}`, anti-traversée par `Path.name`).
  - `installer()` : **AppImage** remplacement atomique (`os.replace`, aucun droit) ; **Windows Setup** → `.bat` silencieux qui attend la fermeture puis `/VERYSILENT /SUPPRESSMSGBOXES /NORESTART` ; **Windows onefile** → `.bat` qui remplace `sys.executable` et relance ; **Linux `.deb`** → `pkexec dpkg -i` après fermeture puis relance `/opt/gestion-scolaire/gestion-scolaire`. Processus **détachés** (`start_new_session` / `CREATE_NO_WINDOW`), scripts dans `mises_a_jour/`.
  - `blocage_obligatoire()` : consigne serveur `"obligatoire": true` → boîte d'info à clic unique (pas de « Non »).
- **Endpoints serveur (`server/main.py`, sécurisés par `securite.sync_autorisee`)** :
  - `GET /mise-a-jour/etat` → `{"actif": bool, "version"?, "obligatoire"?, "paquet_linux"?, "paquet_windows"?, "sha256"?}` (actif=false par défaut).
  - `GET /mise-a-jour/paquet/{nom_paquet}` → fichier du dépôt central (404 sinon).
- **UI (`ui/updater_ui.py`)** : `verifier_au_demarrage()` (silencieux, n'affiche rien si rien/indisponible), `verifier_manuel()` (bouton Paramètres), boîte Oui/Non avec source + versions, `QProgressDialog` indéterminé pendant le téléchargement, messages selon l'action d'installation.
- **Câblage (`main.py`)** : une seule vérification silencieuse par lancement, déclenchée `QTimer.singleShot(3000)` après l'affichage (jamais avant la connexion) ; garde-fous `window.isVisible()`/`RuntimeError` si la fenêtre a été fermée avant le minuteur.
- **Paramètres (`parametres_page.py`)** : groupe « Mises a jour » — bouton « Vérifier les mises à jour » (tous) ; **dépôt central visible seulement si hôte** : choix du paquet + version cible (semver validée, paquet obligatoire avant activation), boutons « Activer la consigne » / « Désactiver ». L'activation diffuse aussi l'empreinte SHA-256 du paquet déposé → intégrité vérifiée par les postes.
- **Tests (25 nouveaux, tous verts)** : `tests/test_updater.py` (versions, choix du paquet, consigne/dépôt, téléchargement SHA-256/progression/erreurs, sources A/B avec `SYNC_ACTIVE` pour `cible_serveur` et `api.client._request` mocké via `importlib.import_module` — piège : le package définit un singleton `client = ApiClient()` qui écrase l'attribut `api.client` du paquet, d'où un `monkeypatch` par chaîne impossible) et `server/test_routes_mise_a_jour.py` (fixture SQLite type `test_routes_exercice.py`).
- **Chasse aux bugs** (demande utilisateur « règle tous les bugs possibles ») :
  - **Bug réel corrigé** : `APP_VERSION` utilisé sans import dans `ui/pages/parametres_page.py` (groupe Mises à jour) → `NameError` au chargement de l'onglet Systeme ; import ajouté.
  - Import inutile `time` retiré de `services/updater.py`.
  - Garde-fous `RuntimeError`/`window.isVisible()` dans `main.py` et `ui/updater_ui.py` (fermeture rapide pendant téléchargement/vérification).
  - `pyflakes` sur `api/ core/ database/ repositories/ services/ ui/ main.py build_app.py server/` : **aucun « undefined name »**.
- **Passe « bugs et logique » (session XXXVI, suite)** :
  - **Détection AppImage fiabilisée** : une AppImage montée par FUSE ne fait pas finir `sys.argv[0]` par `.appimage` (le binaire exécuté est `AppRun`) → `est_appimage()` s'appuie désormais sur la variable d'environnement `APPIMAGE` (source fiable du runtime) avec repli sur `argv[0]` ; `remplacer_appimage()` remplace l'**archive** (env `APPIMAGE` ou `argv[0]`), jamais l'overlay monté.
  - **Intégrité du dépôt central** : l'empreinte SHA-256 calculée par `deposer_paquet()` est désormais **diffusée dans la consigne** (`sha256: {<nom>: <hex>}`) ; les postes vérifient le paquet téléchargé depuis le serveur comme ils le font pour GitHub (`telecharger` vérifie déjà le SHA si fourni).
  - **Consigne cohérente** : `_activer` refuse une version non-semver (**validation `version_cle`**) et refuse d'activer **sans paquet déposé** (sinon les postes recevraient une consigne « cassée » → 404) ; `desactiver_consigne()` purge les restes (`paquet_linux/paquet_windows/sha256`), pas seulement `actif`.
  - **Cohérence paquet/plateforme dans `installer()`** : jamais de `.deb` appliqué sur Windows ni de `.exe` sur Linux (message d'erreur explicite) ; sur Linux le `.deb` ET le `.AppImage` restent acceptés.
  - **Scripts d'installation** : style découplé dans `_style_script()` — Windows `\r\n` + **cp1252** (cmd.exe ne lit pas l'UTF-8), Linux `\n` + UTF-8 (testable sans pathlib).
  - **Tests ajoutés** (`tests/test_updater.py`, 34 tests updater total) : `est_appimage` via `APPIMAGE`, consigne SHA-256, purge à la désactivation, refus de croiser les formats de paquet, lancement des scripts par plateforme, `_style_script`.
- **Vérifications** : `py_compile` OK sur tous les fichiers touchés ; **suite complète `pytest tests/ server/ -q` → 434 passed** (beat 313 initiaux + 25 nouveaux + re-tests) en ~4 min ; les redéfinitions `hacher/verifier_mot_de_passe` signalées dans `server/main.py` sont un artefact délibéré de structure (deux blocs) — sans impact.

## Session XXXVII — Nettoyage UI des textes d'aide secondaires + correctifs CI + version 1.6.3
- **Demande utilisateur** : « enlève tous les commentaires / textes secondaires inutiles dans l'ensemble de l'application » puis « ensuite règle les bugs ». Niveau choisi : **« Équilibré »** — retirer les sous-titres descriptifs de sections, les paragraphes d'aide grisés (`STYLE_HELP_MUTED`), les 2es phrases d'explication des listes vides et les tooltips-paragraphes ; **conserver** les placeholders de saisie, les libellés de champs, les statuts dynamiques (« En Ligne », « Aucun modèle sélectionné »…) et les tooltips courts d'action (ex. « Ouvre l'assistante Charo. »).
- **`ui/widgets/page_header.py`** : `PageHeader` réduit à `(titre)` (plus de `sous_titre` ; `set_sous_titre` et le label d'accroche supprimés → import `Qt` retiré). **`ui/widgets/page_templates.py`** : `_BaseTemplate` sans sous-titre, `remplir()` sans paramètre `sous_titre_vide`. **`ui/pages/helpers.py`** : `_page_header(parent_lay, titre)` simplifié (+ import `STYLE_HEADER_SUBTITLE` retiré).
- **Pages passées au crible** (sous-titres de section, 2es phrases de vides, descriptions de listes) : dashboards, caisse, presences, eleves (bloc `set_sous_titre` sorti), comptes, planning, tarifs, personnel, classes, paiements, bloc_notes, reseau, documents, calendrier, cycles, programmes, notes (3 onglets), statistiques, rapports (liste `RAPPORTS` réduite à `(nom, titre)`, QLabel `desc` retiré).
- **Paramètres** : 6 paragraphes d'aide retirés (`lbl_sync`, `lbl_reseau_help`, `lbl_maj_help`, `lbl_maj_help_depot`, `lbl_embleme_help`, `lbl_app_help`) — les statuts dynamiques sont conservés (`lbl_backups_empty`, `lbl_reseau_code`, `lbl_maj_depot`, `lbl_seuil`) ; rubrique « Depot central du serveur » gardée ; import `APP_VERSION` devenu inutile retiré.
- **Configurateur graphique** : 9 paragraphes/intros retirés (thème global, apercu en direct, composants, sidebar, icônes fa5s, réglages locaux, mémo Export/Import) ; le réglage « Sous-titre de page » (thème) est conservé (réglage fonctionnel).
- **assistant_serveur** : 3 paragraphes retirés (sous-titre « Plusieurs ordinateurs… », explication du code école, description mode client) ; « Activez la connexion pour obtenir l'adresse. » conservé (état initial du label). **login_view** : `_EcranAuth(titre)` sans `sous_titre` ; LoginDialog (« Connexion ») et FirstSetupDialog (« Bienvenue ») ajustés ; le retour en direct « Votre identifiant sera : … » est conservé (feedback dynamique). **main_view** : 3 tooltips de statut raccourcis (Simplifié) ; `_conseil_page` (palette Ctrl+K) conservé (aide de recherche fonctionnelle). **assistant_page** : sous-titre « Votre assistante de gestion scolaire » retiré. **modeles_documents** : « Aucun modele selectionne. Cliquez sur « Nouveau ». » → « Aucun modele selectionne ».
- **Bilan** : 29 fichiers modifiés, **−321 lignes** de textes d'aide ; `py_compile` + `pyflakes` OK (imports morts préexistants non touchés, hors ceux rendus morts par le nettoyage).
- **Correctifs de la branche 1.6.2 (vus en CI) reperés et corrigés ici** : `services/updater.py` écrit désormais les scripts d'installation en **binaire** (`write_bytes`) — `write_text` convertissait `\n` en `\r\n` sous Windows (scripts `.bat` en double CRLF) ; tests `test_updater.py` rendus portables Windows (`test_cible_serveur_active` nom de paquet selon `sys.platform`, `test_ecrire_script_linux_lf_executable` passe `tmp_path`).
...
> **Déjà traités** dans le commit `69bea28` : (a) `RoleAuthorizer` (`ctx.can_edit("eleves")` sur création / modification / suppression), (b) tombstones contournés (`est_supprime = 0` sur `/eleve_recherche`, `/parents_par_classe`, `/eleve-syndication`), (e) édition d'un élève sans classe, plus 3 moyens : recherche locale insensible casse/accent (UDF `sans_accents`), vocabulaire `statut` serveur traduit vers l'UI, neutralisation des formules CSV (`services/csvsafe.py`). Restent donc (c), (d), (f) et les moyens/mineurs non cités ci-dessous.
...
1. **v2 Charo** : script d'évolution **sans téléchargement de modèle lourd** (pas de llama-cpp / Ollama / Qwen) — branché sur le moteur local existant. **Phase P1 livrée** (§10), reste P2 → P6.
2. Release **1.6.5** (bugs + IA) puis fusion de `fix/updater-timeout` vers `main` (main est toujours en 1.5.0).

### 10. Charo v2 — phase P1 : routeur d'intentions déclaratif
- **Problème** : `traiter()` essayait les 12 handlers métier dans un **ordre fixe** ; une question hors du premier choix « pertinent » tombait à côté (ex. « l'état de la caisse » se faisait voler par un handler d'absences), et ajouter une capacité métier imposait de toucher au tuple.
- **Livré** : `services/ia/intentions.py` — registre déclaratif des 12 intentions (mots-clés « clés » discriminants + mots d'appoint + synonymes + regex, poids, handler, exemple), scoring explicite (1 clé 0.60 · 2 clés 0.80 · 2 mots 0.50 · 1 mot 0.30 · regex +0.25, borné à 1.0), seuil de rétention 0.45, seuil de confiance 0.80, égalités départagées par ordre de registre, `expliquer()` pour la trace. Normalisation casse/accents via `ia/langue.normaliser`, recherche en mot entier (`\b`).
- **Intégration non intrusive** : `HANDLERS_METIER` (tuple de noms) reste l'ordre historique de repli ; le routeur n'est appelé qu'au-dessus du seuil de confiance et **court-circuite** le handler choisi. Si ce handler renvoie `None` ou lève, on rebascule sur l'ordre fixe → **0 régression** par construction. Aucun handler métier réécrit.
- **Feature flags** (`core/config.py`, dict `CHARO_V2`) : `ROUTEUR` (défaut `true`), `SEUIL_INTENTION`, `SEUIL_CONFIANT`, surchargeables par `GS_CHARO_ROUTEUR` / `GS_CHARO_SEUIL_INTENTION` / `GS_CHARO_SEUIL_CONFIANT` — retour v1 sans toucher au code.
- **Tests** : 39 tests unitaires (`tests/test_intentions.py` : barème, accents/casse, mot entier, regex, hors périmètre, tie-break, intégrité registre↔handlers) + 16 tests d'intégration dans `tests/test_assistant_ia.py` (non-régression v1/v2 réponse par réponse sur 10 questions types, ordre d'appel vérifié pour le court-circuit, repli sur handler `None`, flag désactivé, hors périmètre). Régression : `test_assistant_ia.py` + `test_intentions.py` **117 passed**, IA complète + `server/` **verts**.
- **Suite** : P2 (index BM25 + synonymes), P3 (raisonnement traçable), P4 (mémoire renforcée), P5 (graphe déductif), P6 (batterie de 100 questions + rapport) — détail dans `docs/PROJET_CHARO_IA_LOCALE.md`.
